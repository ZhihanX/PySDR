...
num_samps = 10000 
sdr_server = adi.Pluto("ip:192.168.2.1")
...
samples = np.concatenate((samples1,samples0, samples2,samples0,samples3,samples0,samples4,samples0,samples5))
...
# sdr_sensor.rx_buffer_size = num_samps*4
...
print("power:",np.average(x_shaped))
plt.figure(5)
plt.plot(h)
plt.xlabel("time")
plt.figure(2)
...
# value = max(x_shaped)
# max_index = np.argmax(x_shaped)
# print(max_index)
# # Print the array with the maximum value in the middle
# plt.figure(8)
# plt.plot(x_shaped[max_index-5000:max_index+5000])
sdr_server.tx_destroy_buffer()
sdr_server.tx(x_shaped)

######windows for each pulse##########
peaks, _ = signal.find_peaks(x_shaped,distance=1600)
print("peak:",peaks)
widths = signal.peak_widths(x_shaped, peaks, rel_height=0.15)
avg_p = 0
for i in range(5):
    avg_p += np.average(x_shaped[int(widths[2][i]):int(widths[3][i])])
avg_p = avg_p/5
print ("avarage:",avg_p)
print("width:",widths[2])
plt.figure(10)
plt.plot(x_shaped)
...
# peaks, _ = signal.find_peaks(x_shaped,distance=1600)
# prominence = signal.peak_prominences(x_shaped,peaks)[0]
# print(prominence)
plt.show()
try:
    while True:
...
# rx_samples.real = rx_samples.real*rx_samples.real/10
# rx_samples.imag = rx_samples.imag*(-rx_samples.imag)/10
